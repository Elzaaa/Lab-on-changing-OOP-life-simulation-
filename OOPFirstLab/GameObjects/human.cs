...
{
    /// <summary>
    /// Поведение человека значительно отличается от прочих
    /// </summary>
    public class Human : GameObject
    {
        public static Human Create(IGameEngine gameEngine)
        {
            bool isMutant = gameEngine.GetRandom().Next(10) < 2;
            Gender g = (gameEngine.GetRandom().Next() % 2 == 0) ? Gender.Male : Gender.Female;
            return new Human(gameEngine, isMutant, g);
        }

        private Human(IGameEngine gameEngine, bool isMutant, Gender g) : base(gameEngine, GameObjectType.Human, "Human.xml")
        {
            IsMutant = isMutant;
            Gender = g;
        }

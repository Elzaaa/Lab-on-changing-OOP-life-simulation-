...
namespace OOPFirstLab
{
    public partial class Form1 : Form
    {
        ...
        private void DrawCurrentState()
        {
            ...
            IGameMap CurrentMap = gameEngine.GetCurrentMap();
            for (int h = 0; h < CurrentMap.Height; h++)
            {
                for (int w = 0; w < CurrentMap.Width; w++)
                {
                    List<IGameObject> gameObjects = CurrentMap.GetObjectsAtPos(w, h);
                    if (gameObjects != null)
                    {
                        int brushIndex = -1;
                        bool isMutant = false;
                ...
                if (go != null)
                {
                    switch (go.Type)
                    {
                        ...
                        case GameObjectType.HerbivoreAnimal1:
                            sb.AppendFormat("Лось со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
                        case GameObjectType.HerbivoreAnimal2:
                            sb.AppendFormat("Олень со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
                        case GameObjectType.HerbivoreAnimal3:
                            sb.AppendFormat("Кролик со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
                        case GameObjectType.OmnivoreAnimal1:
                            sb.AppendFormat("Медведь со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
                        case GameObjectType.OmnivoreAnimal2:
                            sb.AppendFormat("Енот со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
                        case GameObjectType.OmnivoreAnimal3:
                            sb.AppendFormat("Пиг со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
                        case GameObjectType.PredatoryAnimal1:
                            sb.AppendFormat("Тигр со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
                        case GameObjectType.PredatoryAnimal2:
                            sb.AppendFormat("Гепард со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
                        case GameObjectType.PredatoryAnimal3:
                            sb.AppendFormat("Волк со здоровьем {0}", ((GameObject)go).Health).AppendLine();
                            break;
